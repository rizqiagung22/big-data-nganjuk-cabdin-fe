...
} from "@/modules/admin/stores/admin.pendistribusian-ijazah.store.ts";
...
const handleDownload = async (item : any, _ : string) => {
  try {
    await  getDownloadFileApiPendistribusianIjazah(item.id, item.name_file );
  } catch (error : any){
    if(error.status == 404){
...
const handleRemove = async (item : any, type : string) => {
  try {
    await  deleteFileApiPendistribusianIjazah(item.id );
    await fetchData();
  } catch (error : any){
    if(error.status == 404){
...
const handleUpload = async (event: Event, item : any, _ : string, idHtml : string) => {
  ...
  try {
    ...
    if (target.files && target.files.length > 0) {
      ...
      await updateFileApiPendistribusianIjazah(item.id, formData);
      await fetchData();
    }
  } catch (error : any){
